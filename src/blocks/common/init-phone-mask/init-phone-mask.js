app.modules.initPhoneMask = (function (self) {
  var CONFIG = app.config.initPhoneMask,
  ...
  function _initPhoneMask(currentValue, event, $field) {
    ...
    $field.mask(
      _options.onlyDefaultCode
        ? _options.defaultPattern.replace('+0', DEFAULT_CODE)
        : _options[pattern],
      _options.params
    );
  ...
  function _listener() {
    $doc.on('init:phoneMask', function (event) {
      _init(event.target !== document && event.target);
    });
  }
